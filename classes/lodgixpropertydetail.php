...
class LodgixPropertyDetail
{
    ...
    {
        if($tabVersion == 3){
            $layoutClass = 'ldgxPropertySingleWrapperNew';
            $header = $this->headerAlt();
        } else {
            $layoutClass = '';
            $header = $this->header();
        }
        ...
        $amenities = $showAmenities ? $this->amenities($tabVersion) : '';
        ...
        $html = <<<EOT
<div id="top" class="ldgxPropertySingle ldgxPropertyWrapper ldgxPropertySingleWrapper $layoutClass">
    $header
    $slider
    $buttonCheckAvailability
    $description
    ...
    $taxesFeesDepositsPolicies
    $map
    <a id="ldxToTop" href="#top"><span>Back To Top</span></a>
...
EOT;
    ...
    {
        $html = '<ul class="ldgxAnchorMenu">';
        $html .= $this->menuHeader($tabTitleDetails, 'lodgix_anchor_details', 'ldgxTabDetails', '#details');
        $html .= $this->menuHeader($tabTitleCalendar, 'lodgix_anchor_booking_calendar', 'ldgxTabCalendar', '#booking');
        $html .= $this->menuHeader($tabTitleLocation, 'lodgix_anchor_location', 'ldgxTabLocation', '#map');
        $html .= $this->menuHeader($tabTitleAmenities, 'lodgix_anchor_amenities', 'ldgxTabAmenities', '#amenities');
        $html .= $this->menuHeader($tabTitlePolicies, 'lodgix_anchor_policies', 'ldgxTabPolicies', '#policies');
        $html .= $this->menuHeader($tabTitleReviews, 'lodgix_anchor_reviews', 'ldgxTabReviews', '#reviews');
        $html .= '</ul>';
        $html .= <<<EOT
<script>
    var hashTagActive = "";
   jQueryLodgix(".ldgxAnchorMenu li a").add('#ldxToTop').on("click touchstart" , function (event) {
       if(hashTagActive != this.hash) { //this will prevent if the user click several times the same link to freeze the scroll.
           event.preventDefault();
           //calculate destination place
           var dest = 0;
           if (jQueryLodgix(this.hash).offset().top > jQueryLodgix(document).height() - jQueryLodgix(window).height()) {
               dest = jQueryLodgix(document).height() - jQueryLodgix(window).height();
           } else {
               dest = jQueryLodgix(this.hash).offset().top;
           }
           //go to destination
           jQueryLodgix('html,body').animate({
               scrollTop: dest
           }, 500, 'swing');
   ...
   jQueryLodgix(window).scroll(function() {
       var scroll = jQueryLodgix(window).scrollTop();
       if (scroll >= 500) {
           //clearHeader, not clearheader - caps H
           jQueryLodgix("#ldxToTop").addClass("shown");
       }
   });
...
EOT;
    ...
    protected function menuHeader($tabTitle, $id, $class, $href)
    {
        $html = '';
        if ($tabTitle || $this->displayHiddenTabs) {
            $label = LodgixTranslate::translate($tabTitle);
            $html .= <<<EOT
<li id="$id" class="ldgxAnchorTab $class">
    <a href="$href">$label</a>
</li>
EOT;
        }
    ...
    {
        if($tabVersion == 2){
            $layoutClass = 'ldgxPropertyTabbedWrapperNew';
            $header = $this->headerAlt(true);
        } else {
            $layoutClass = '';
        ...
        $tabs = $this->tabsTemplate(
            ...
            $this->calendar(),
            $this->map(),
            $this->amenities($tabVersion),
            $this->ratesTaxesFeesDepositsPolicies(),
            $this->reviews()
        );

        $html = <<<EOT
<div class="ldgxPropertyTabs ldgxPropertyWrapper ldgxPropertyTabbedWrapper $layoutClass">
    $header
...
EOT;
    ...
    function headerAlt($isTabs = false)
    {
        $description = $this->property->description;
        $bedrooms = $this->property->bedrooms ? $this->property->bedrooms . ' ' . LodgixTranslate::translate('Bedrooms') : LodgixTranslate::translate('Studio');
        $bathrooms = $this->property->bathrooms . ' ' . LodgixTranslate::translate('Bathrooms');
        ...
        $class = LodgixConst::$ICON_SET_CLASS[$this->iconSet];
        $html = <<<EOT
<div class="ldgxPropBadge ldgxPropBadgeClean">
    <div class="ldgxPropBadgeCleanColumn">
        <div class="ldgxPropBadgeTitle">
            <span class="ldgxPropBadgeName">$description</span>
            <div class="ldgxPropBadgeRooms">
                <span class="ldgxPropBadgeRoomsBeedrooms">$bedrooms</span>
        ...
        <div class="ldgxPropBadgeIconsGroup">
            $mapIcon
            $videoIcon
            $virtualTourIcon
            $emailIcon
...
EOT;
    ...
    {
        $html = '';
        if ($this->displayRatesMonthly && $this->ratesMonthly && $this->ratesMonthly[0] != null) {
            $from = LodgixTranslate::translate('from');
...
EOT;
        foreach ($this->photos as $photo) {
            ...
            $html .= <<<EOT
            <img class="rsImg" src="$previewUrl" data-rsBigImg="$previewUrl" data-rsTmb="$photo->thumb_url" alt="$caption" />
EOT;
        }
        $html .= <<<EOT
        </div>
...
<div class="ldgxPropertyBlock ldgxPropertyBlockCheckAvailability">
    <a title="$label" href="$url"><img src="$pluginUrl/images/booknow.png" alt="Book Now"></a>
</div>
EOT;
        } else {
...
EOT;
    ...
    function description()
    {
        ...
        if ($this->property->description_long) {
            ...
            $html .= <<<EOT
<div id="ldgxpropertydescription" class="ldgxPropertyBlock ldgxPropertyBlockDescription">
    <h2>$label</h2>
...
EOT;
    ...
    function details($inject1 = '', $inject2 = '')
    {
        ...
        $html = <<<EOT
<div id="details" class="ldgxPropertyBlock ldgxPropertyBlockDetails">
    <h2>$label</h2>
...
EOT;
    ...
    function amenities($tabVersion)
    {
        ...
        if (count($amenityCategories)) {
            ...
            if( $tabVersion == 2 || $tabVersion == 3 ){
                $amenities .= '<div class="ldgxPropertyBlockAmenitiesGrid">';
                foreach ($amenityCategories as $amenityCategory) {
                    $amenities .= $this->amenityCategoryIcons($amenityCategory);
                }
                $amenities .= '</div>';
            } else {
                foreach ($amenityCategories as $amenityCategory) {
            ...
            $html .= <<<EOT
<div id="amenities" class="ldgxPropertyBlock ldgxPropertyBlockAmenities">
    <h2>$label</h2>
    $amenities
</div>
EOT;
...
EOT;
    ...
    function amenityCategoryIcons($amenityCategory){
        $html = '';
        $amenityNames = $amenityCategory['amenity_names'];
        if (count($amenityNames)) {
            $categoryName = LodgixTranslate::translateTerm($amenityCategory['category_name'], $this->language);
            $categoryClass = str_replace(' ', '-', strtolower($categoryName));
            $amenities = '';
            foreach ($amenityNames as $amenity) {
                $amenities .= '<li>' . LodgixTranslate::translateTerm($amenity, $this->language) . '</li>';
            }
            $html .= <<<EOT
<div class="ldgxPropertyBlockAmenitieGroup $categoryClass">
<h3>$categoryName</h3>
...
EOT;
    ...
    function reviews()
    {
        ...
        if (count($this->reviews)) {
            ...
            $html .= <<<EOT
<div id="reviews" class="ldgxPropertyBlock ldgxPropertyBlockReviews">
    <h2>$label</h2>
    $reviews
</div>
EOT;

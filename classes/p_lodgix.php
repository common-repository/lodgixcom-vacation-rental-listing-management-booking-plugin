...
class p_lodgix
{
    ...
    public function __construct()
    {
        ...
        //send information about plugin version
		add_action( 'admin_post_nopriv_ldx_version', array(&$this, 'ldx_plugin_version_report') );
		add_action( 'admin_post_ldx_version', array(&$this, 'ldx_plugin_version_report') );
    ...
    function ldx_plugin_version_report(){
        if($_GET['action'] == 'ldx_version'){
            ...
            echo json_encode(array(
                'ldx_version' => '1',
                'plugin_version' => LodgixConst::PLUGIN_VERSION
            ));
        }
    }
    ...
    function load_js_and_css_jquery()
    {
        ...
        wp_enqueue_script('lodgix_jquery_no_conflict_pre_js', $path . 'js/jquery-no-conflict-pre.js', array(), LodgixConst::PLUGIN_VERSION);

        wp_enqueue_script('lodgix_jquery_latest_js', $path . 'js/jquery-v-1.11.3.min.js', array(), LodgixConst::PLUGIN_VERSION);
    ...
    function load_js_and_css_jquery_lodgix()
    {
        ...
        wp_enqueue_style('lodgix_inventory_grid3_css', $path . 'css/inventory/grid3.css', array(), LodgixConst::PLUGIN_VERSION);
        ...
        wp_enqueue_style('lodgix_widgets_featuredv2_css', $path . 'css/widgets/featuredv2.css', array(), LodgixConst::PLUGIN_VERSION);
    ...
    function load_admin_js_and_css($hook){
        ...
        if (is_admin() && ($hook == 'settings_page_lodgix' || $hook == 'settings_page_lodgix-upgrade')) {
        ...
        wp_enqueue_script( 'wp-color-picker-alpha', plugins_url( '../js/wp-color-picker-alpha.js',  __FILE__ ), array( 'wp-color-picker' ), '2.1.4' );
        wp_enqueue_media();
...
//            $content = str_replace('[lodgix search_rentals]',$this->get_search_rentals_page_content(),$content);
    ...
    function admin_menu_link()
    {
        ...
        add_options_page( 'Lodgix Upgrade', 'Lodgix Upgrade', 'manage_options', 'lodgix-upgrade', array(&$this, 'admin_upgrade_page'));
    ...
    function p_lodgix_geturls()
    {
        ...
        foreach ($posts as $post) {
            ...
            //    $langURLs[] = array(
            //        'code' => strtoupper($lpost->language_code),
            //        'url' => htmlentities(urlencode(get_permalink($lpost->page_id)))
            //    );
            //}
            $data['properties'][] = array(
                'id' => $post->property_id,
                'url' =>  htmlentities(urlencode(get_permalink($post->page_id))),
                'languages' => $langURLs
            );
        }
    ...
    function admin_upgrade_page(){
        ...
        //include 
        include plugin_dir_path( __FILE__ ) . 'lodgixupgradepage.php';

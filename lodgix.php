...
Plugin Name: Lodgix.com Vacation Rental Website Builder
...
Changelog
v3.9.73: Updated file structure for update page.
v3.9.72: 5.9 testing. Updated readme. Added additional informatin about the newest lodgix plugin https://www.lodgix.com/vacation-rental-wordpress/.
v3.9.71: 5.7 testing. Updated readme.
v3.9.70: Updated additional widget code to help prevent issues in error log.
v3.9.69: Updated widget code to help prevent issues in error log.
v3.9.68: Fixed an issue with bed counts.
v3.9.67: Added the display of room types, and bed counts
v3.9.66: Added the display of bed types within bedrooms
v3.9.65: Added version information endpoint
v3.9.64: Fixed display options for new grid. Updated grid output order.
v3.9.63: Better optimized images on individual property pages.
v3.9.62: Reset Styles button added to search widget. Added option to hide category titles on grid layout.
v3.9.61: Improved reset styles button in featured rentals widget. Set default width of search widget form to 100%. Added category organization to new grid layout. Hid tabs on mobile within single page layout.
v3.9.60: Improved customization fields of the search and featured widgets. Added alt tags to images from Lodgix.
v3.9.59: Added styles to search widget to evenly distribute fields when displayed horizontally and to respond better on smaller screens. Fixed bug with color picker, added style reset button, and added additional style fields to the featured properties widget.
v3.9.58: Updated state value output on grid layout.
v3.9.57: Consolidated featured widget customization options. Updated readme description.
v3.9.56: Single property style update
v3.9.55: Updated styles on new featured widget, single property tabbed layout and single property full page layout. Updated default values in new featured widget.
v3.9.54: Removed old version of search widget. Added a new featured rentals widget with multiple styling options. Renamed existing featured rentals widget "Lodgix Featured Rentals - Classic". Renamed plugin from "Lodgix.com Vacation Rental Listing, Management & Booking Plugin" to "Lodgix.com Vacation Rental Website Builder". Added Lodgix thumbnail and Banner Image to plugin repo.
v3.9.53: New controls were added for setting up the grid layout. Improved UI styles of the Lodgix settings screen. Added styles to fix text wrapping in the grid layout.
v3.9.52: Bug fixes and improvements.
v3.9.51: Bug fixes and improvements.
v3.9.50: Added UI improvements.
v3.9.40: Bug fixes and improvements.
v3.9.39: Improvement to avoid conflicts with other plugins and themes.
v3.9.38: Fixed an issue between Divi Builder and Lodgix plugin.
v3.9.37: Changed fetch property URLs to JSON.
v3.9.36: Changed plugin version.
v3.9.35: Strip tags from property listings description.
v3.9.34: Improved accessibility in the search widget.
...
include_once dirname(PLUGIN_PATH) . '/lodgix_widget_featured_rentals_v2.php';
...
//     if ( isset($_GET['page']) && $_GET['page'] == 'lodgix.php') {
//         echo '<div class="notice notice-warning">
//                 <p>There is a new Lodgix plugin. Please visit <a href="https://www.lodgix.com/vacation-rental-wordpress/" target="_blank">https://www.lodgix.com/vacation-rental-wordpress/</a> for more details.</p>
//             </div>';
//     }
// }
...
function lodgix_plugin_download_link(){

    $link = 'https://www.lodgix.com/vacation-rental-wordpress/';

    $request = wp_remote_request( 'https://kernl.us/api/v1/latest-version/5f7f67def946b7614ff8102c', array(
		'method' => 'GET',
		'timeout' => 7,
	));
    ...
    if($data){
        ...
        if(isset($data->downloadUrl)){
            $link = sanitize_text_field($data->downloadUrl);
        }
 ...
 * Activate the plugin.
 */
function lodgix_v1_activate( $plugin ) {
    if( $plugin == plugin_basename( __FILE__ ) ) {
        exit( wp_safe_redirect( admin_url( 'options-general.php?page=lodgix-upgrade' ) ) );
    }
}
add_action( 'activated_plugin', 'lodgix_v1_activate' );